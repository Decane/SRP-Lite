...
- Fixed the issue where a 'help' task would get cancelled instead of completed if the player kills the attack squad before it leaves for its target point (i.e. before the timer reaches zero). - Decane
...
- Fixed the inconsistent fonts of some sub-headings in the credits. - SurDno, Decane
